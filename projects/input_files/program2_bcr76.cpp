...
//
// This program takes an input file of movie data and gives the user options to view the movies
// sorted by title or rating and find movies based on certain criteria.
...
const int MAX_MOVIE_ARRAY_SIZE = 100;
...
int main() {
    Movie moviesData[MAX_MOVIE_ARRAY_SIZE]; // array of Movie structs to store up to 100 movies read from the input file
    int numberOfMovies = getMovieData(moviesData, MAX_MOVIE_ARRAY_SIZE); // integer variable to hold the true number of movies that are read in
...
int getMovieData(Movie moviesData[], int SIZE){
    ...
    data.open("./library.txt"); // change this path as needed, it will look for the input file in current directory
    if(!data){
